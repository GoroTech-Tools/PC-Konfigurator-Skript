...
function Get-QuickAccessFilesystemEntries {
    ...
    $entries = foreach ($qaItem in @($quickAccessNamespace.Items())) {
        ...
        try { $path = $qaItem.Path } catch { $null = $_.Exception.Message }
        try { $name = $qaItem.Name } catch { $null = $_.Exception.Message }
...
function Remove-StaleDesktopQuickAccessEntries {
    ...
    foreach ($qaItem in @($quickAccessNamespace.Items())) {
        ...
        try { $itemName = $qaItem.Name } catch { $null = $_.Exception.Message }
        try { $itemPath = $qaItem.Path } catch { $null = $_.Exception.Message }
